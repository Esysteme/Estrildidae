...
//Fatal error: Allowed memory size of 134217728 bytes exhausted (tried to allocate 8192 bytes) in /home/www/species/repository/glial/Glial/Sgbd/Sql/Sql.php on line 114
        $sql = "SELECT id_nominal , a.nominal,b.id_species_sub,   b.language, b.text as name, d.total_found
        ...
        while ($ob = $this->db['mysql_write']->sql_fetch_object($res)) {
            ...
            echo Color::getColoredString("Memory peak  : " . round(memory_get_peak_usage() / (1024 * 1024), 2) . " Mo", "white", "blue") . "\n";
            echo Color::getColoredString("Memory usage : " . round(memory_get_usage() / (1024 * 1024), 2) . " Mo", "white", "blue") . "\n";
            ...
            if ($id_search) {
                ...
                foreach ($data['link_photo'] as $url_to_get) {
                    ...
                    $url_to_get['author']['key'] = $this->db['mysql_write']->sql_real_escape_string($url_to_get['author']['key']);


                    $sql = "SELECT `id`,`surname` FROM `species_author` WHERE `key` ='" . $url_to_get['author']['key'] . "'";
                    $res22 = $this->db['mysql_write']->sql_query($sql);

                    if ($this->db['mysql_write']->sql_num_rows($res22) === 1) {

                        $ob22 = $this->db['mysql_write']->sql_fetch_object($res22);

                        $author = array();
                        $author['species_author']['id'] = $ob22->id;
                        $author['species_author']['key'] = $url_to_get['author']['key'];
                        $author['species_author']['surname'] = $url_to_get['author']['name'];
                        ...
                        echo Color::getColoredString("Added new author(" . $id_author . ")", "white", "black") . ":";
                    }
                    ...
                    if (!$id_picture) {
                        ...
                        //echo "\n" . Color::getColoredString("Memory peak  : " . round(memory_get_peak_usage(), 2) . " Mo", "yellow", "black") . "\n";
                        //echo Color::getColoredString("Memory usage : " . round(memory_get_usage(), 2) . " Mo", "yellow", "black") . "\n";
    ...
    function updateAuthor()
    {
        ...
        while ($ob = $this->db['mysql_write']->sql_fetch_object($res)) {
            ...
            $i++;
            $tab_key = explode('/', $ob->url_context);

            $sql = "UPDATE `species_author` SET `key`='" . $tab_key[4] . "', `id_species_source_main`=7 where `id`=" . $ob->id_species_author . "";
